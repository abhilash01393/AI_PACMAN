    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                cost = 1
                corners = set(state[1])
                if newState in self.corners and newState in corners:  
                    corners.remove(newState)
                successors.append(((newState, tuple(corners)), action, cost))
...
def cornersHeuristic(state, problem):
    ...
    for corner in corners:
        if (max_corner_dist == -1) or (max_corner_dist < util.manhattanDistance(corner, state[0])): 
    ...
    return max_corner_dist   # Default to trivial solution
...
def foodHeuristic(state, problem):
    ...
    for curr_node in foodGrid.asList():
        pos_problem = PositionSearchProblem(problem.heuristicInfo['state'], start=position, goal=curr_node, warn=False, visualize=False)
        food_nodes_dist.append(len(search.bfs(pos_problem)))  

    if len(food_nodes_dist) == 0:
        return 0
    max_food_nodes_dist = max(food_nodes_dist)
    return max_food_nodes_dist
