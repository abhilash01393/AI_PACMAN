...
def axiom_generator_percept_sentence(t, tvec):
    ...
    for i in range(len(tvec)):
        if tvec[i]:
            proposition.append(percepts[i] + str(t))
        else:
            proposition.append('~' + percepts[i] + str(t))
    axiom_str = ' & '.join(proposition)
    return axiom_str
...
def axiom_generator_pits_and_breezes(x, y, xmin, xmax, ymin, ymax):
    ...
    "*** YOUR CODE HERE ***"
    pits = []
    for (cor_x,cor_y) in [((x-1),y),(x,(y-1)),((x+1),y),(x,(y+1))]:
        if cor_x >= xmin and cor_x <= xmax and cor_y >= ymin and cor_y <= ymax:
            pits.append(pit_str(cor_x,cor_y))
...
def axiom_generator_wumpus_and_stench(x, y, xmin, xmax, ymin, ymax):
    ...
    for (cor_x, cor_y) in [((x - 1), y), (x, (y - 1)), ((x + 1), y), (x, (y + 1))]:
        if cor_x >= xmin and cor_x <= xmax and cor_y >= ymin and cor_y <= ymax:
            all_wumpus.append(wumpus_str(cor_x, cor_y))
    all_wumpus.append('W'+str(x)+'_'+str(y))
    axiom_str = '{0} <=> ({1})'.format(stench_str(x, y), (' | ').join(all_wumpus))
    return axiom_str
...
def axiom_generator_at_least_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    "*** YOUR CODE HERE ***"
    # Comment or delete the next line once this function has been implemented.
    room_des = [wumpus_str(cor_x,cor_y) for cor_x in range(xmin,xmax + 1) for cor_y in range(ymin,ymax + 1)]

    axiom_str = ' | '.join(room_des)
    return axiom_str
...
def axiom_generator_at_most_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    "*** YOUR CODE HERE ***"
    # Comment or delete the next line once this function has been implemented.
    room_des = [(cor_x, cor_y) for cor_x in range(xmin, xmax + 1) for cor_y in range(ymin, ymax + 1)]
    axioms = []

    for room in room_des:
        neg_room = [thisRoom for thisRoom in room_des if thisRoom != room]
        neg_wumpus = ['~'+wumpus_str(w[0],w[1]) for w in neg_room]
        axioms.append('({0} >> ({1}))'.format(wumpus_str(room[0],room[1]),' & '.join(neg_wumpus)))
...
def axiom_generator_only_in_one_location(xi, yi, xmin, xmax, ymin, ymax, t = 0):
    ...
    "*** YOUR CODE HERE ***"
    # Comment or delete the next line once this function has been implemented.
    curr_room = ('~L'+str(xi)+'_'+str(yi)+'_'+str(t))
    neg_room = ['~' + state_loc_str(xVal,yVal,t) for xVal in range(xmin,xmax+1) for yVal in range(ymin,ymax+1)]
    neg_room.remove(curr_room)
    before_axiom_str = 'L{0}_{1}_{2}'.format(xi,yi,t)
    after_axiom_str = '{0}'.format(' & '.join(neg_room))
    axiom_str = before_axiom_str + " & " + after_axiom_str
    return axiom_str
...
def axiom_generator_only_one_heading(heading = 'north', t = 0):
    ...
    "*** YOUR CODE HERE ***"
    # Comment or delete the next line once this function has been implemented.
    direction_heading = ['North', 'South', 'East', 'West']
    neg_head = []
    for direction in direction_heading:
        if direction.upper() == heading.upper():
            axiom_str += 'Heading{0}{1} & ('.format(direction,str(t))
        else:
            neg_head.append('~Heading' + direction + str(t))
    axiom_str += '{0})'.format('&'.join(neg_head))
    return axiom_str
...
def axiom_generator_at_location_ssa(t, x, y, xmin, xmax, ymin, ymax):
    ...
    for ((cor_x,cor_y),move) in [(((x-1),y),'E'),((x,(y-1)),'N'),(((x+1),y),'W'),((x,(y+1)),'S')]:
        if cor_x >= xmin and cor_x <= xmax and cor_y >= ymin and cor_y <= ymax:
            if move == 'N':
    ...
    loc_suc = state_loc_str(x,y,t+1)
    axiom_str = '{0} <=> ({1})'.format(loc_suc,' | '.join(moves))
...
def axiom_generator_heading_north_ssa(t):
    ...
    t := time
    """
    neg_move = '({0} & ({1} | {2} | {3} | {4} | {5}))'.format(state_heading_north_str(t),action_wait_str(t),action_grab_str(t),
    ...
    axiom_str = '{0} <=> ({1} | {2} | {3})'.format(state_heading_north_str(t+1),neg_move,turn_left,turn_right)
...
def axiom_generator_heading_east_ssa(t):
    ...
    t := time
    """
    neg_move = '({0} & ({1} | {2} | {3} | {4} | {5}))'.format(state_heading_east_str(t),action_wait_str(t),action_grab_str(t),
                                                    action_shoot_str(t),percept_bump_str(t+1),action_forward_str(t))
    ...
    axiom_str = '{0} <=> ({1} | {2} | {3})'.format(state_heading_east_str(t+1),neg_move,turn_left,turn_right)
...
def axiom_generator_heading_south_ssa(t):
    ...
    t := time
    """
    neg_move = '({0} & ({1} | {2} | {3} | {4} | {5}))'.format(state_heading_south_str(t),action_wait_str(t),action_grab_str(t),
                                                    action_shoot_str(t),percept_bump_str(t+1),action_forward_str(t))
    ...
    axiom_str = '{0} <=> ({1} | {2} | {3})'.format(state_heading_south_str(t+1),neg_move,turn_left,turn_right)
...
def axiom_generator_heading_west_ssa(t):
    ...
    t := time
    """
    neg_move = '({0} & ({1} | {2} | {3} | {4} | {5}))'.format(state_heading_west_str(t),action_wait_str(t),action_grab_str(t),
                                                    action_shoot_str(t),percept_bump_str(t+1),action_forward_str(t))
    ...
    axiom_str = '{0} <=> ({1} | {2} | {3})'.format(state_heading_west_str(t+1),neg_move,turn_left,turn_right)
...
def axiom_generator_only_one_action_axioms(t):
    ...
    t := time
    """
    avail_action = ['Forward','Grab','Shoot','Climb','TurnLeft','TurnRight','Wait']
    axioms = []
    for action in range(len(avail_action)):
        other_avail_action = [('~' + x + str(t))for i,x in enumerate(avail_action) if i!=action]
        axioms.append('({0}{1} <=> ({2}))'.format(avail_action[action],str(t),' & '.join(other_avail_action)))
