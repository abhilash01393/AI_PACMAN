...
class PlanRouteProblem(search.Problem):
    ...
    def h(self,node):
        ...
        """
        "*** YOUR CODE HERE ***"
        goal_dist = [manhattan_distance_with_heading(node.state,goal) for goal in self.goals]
        return min(goal_dist)
    ...
    def actions(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        pos_frwrd = ['Forward','TurnRight','TurnLeft']
        neg_frwrd = ['TurnRight','TurnLeft']
...
class PlanShotProblem(search.Problem):
    ...
    def h(self,node):
        ...
        """
        "*** YOUR CODE HERE ***"
        if_loc_wumpus = self.goals
        explorer_locs_allowed = self.allowed
        shooting_spot = []

        for loc_wumpus in if_loc_wumpus:
            for explorer_loc in explorer_locs_allowed:
                if (loc_wumpus[0] == explorer_loc[0]) or (loc_wumpus[1] == explorer_loc[1]):
                    shooting_spot.append(explorer_loc)

        shoot_spot_dist = [manhattan_distance_with_heading(node.state, goal) for goal in shooting_spot]
        return min(shoot_spot_dist)
